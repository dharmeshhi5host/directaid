...
class LanguageStringController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if (isset($request['panel']) && $request['panel'] != null) {

                $language_strings = $language_strings->whereHas('language_screen', function ($query) use ($request) {
                    $query->where('language_screens.app_or_panel', $request['panel']);
                });
            }
            ...
            return DataTables::of($language_strings)
                ->addColumn('app_or_panel', function ($language_strings) {
                    $app_or_panel_or_front = '';
                    if (isset($language_strings->language_screen->app_or_panel)) {

                        if ($language_strings->language_screen->app_or_panel === 0) {
                            $app_or_panel_or_front = config('languageString.admin_panel');
                        }
                        if ($language_strings->language_screen->app_or_panel === 1) {
                            $app_or_panel_or_front = config('languageString.app');
                        }

                        if ($language_strings->language_screen->app_or_panel === 2) {
                            $app_or_panel_or_front = config('languageString.front_panel');
                        }
                    }
                    return $app_or_panel_or_front;
                })
                ...
                ->addColumn('view_screen', function ($language_strings) {
                    return '<a href="' . route('admin.view-language-screen', [$language_strings->language_screen_id]) . '" class="btn btn-icon btn-info waves-effect waves-light" data-toggle="tooltip" data-placement="top" title="' . config('languageString.view') . '"><i class="far fa-eye align-middle"></i></a>';
                })
                ->rawColumns(['action', 'image', 'view_screen'])
                ->make(true);
        }
        $devMode = DB::table('settings')->where('meta_key', 'is_debug_mode')->first();

        return view('admin.languageString.index', ['devMode' => $devMode]);
    }
    ...
    public function updateDeveloperMode()
    {
        $devMode = DB::table('settings')->where('meta_key', 'is_debug_mode')->first();
        $newMode = $devMode->meta_value == 1 ? 0 : 1;
        DB::table('settings')->where('meta_key', 'is_debug_mode')->update(['meta_value' => $newMode]);

        $message = $newMode ? 'API Dev Mode Enabled' : 'API Dev Mode Disabled';

        return redirect()->back()->with(['toast_message' => $message, 'toast_type' => 'success']);
    }
}
